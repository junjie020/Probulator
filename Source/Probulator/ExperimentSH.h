...
public:
	void run(SharedData& data) override
	{
		...
		for (int l=0; l<L; ++l){
			const int levelcount = 2*(l+1)-1;
			const int totalcount = (l+1)*(l+1);
			for (int ii=(totalcount-levelcount); ii<totalcount; ++ii){
				m_shCoeffs[ii] = vec4(shRadiance.data[ii] * A[l], 0.f);
			}
		}

		auto irradiance_coeffs_origin = [&shRadiance](vec3 n)
		{
			float c1 = 0.429043f;
			float c2 = 0.511664f;
			float c3 = 0.743125f;
			...
			//TODO: see shEvaluate function, x = -n.x, y = -n.y
		...
		data.m_directionImage.forPixels2D([&](const vec3& direction, ivec2 pixelPos)
		{
			...
			// vec3 samplesh1 = max(vec3(0.f), irradiance_coeffs_origin(direction) / pi);
			// vec3 samplesh2 = max(vec3(0.f), irradiance_coeffs(direction) / pi);
